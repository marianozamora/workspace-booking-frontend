...
export const spacesService = {
  ...
  getAll: async (): Promise<Space[]> => {
    const response =
      await apiClient.get<ApiResponse<BackendSpace[]>>('/api/v1/spaces');
...
export const bookingsService = {
  ...
  getAll: async (params?: {
    ...
    const response =
      await apiClient.get<PaginatedResponse<BackendBooking>>(url);

    return {
      ...response.data.data,
      data: transformBookingsData(response.data.data.bookings),
    };
  },
